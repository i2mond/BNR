...
class Net(nn.Module):
    ...
    def forward(self, method, x):
        ...
        attn_weights_detach = torch.mean(attn_weights_detach, dim=2)  # 12 * B * N-1 * N-1
        patch_attn = attn_weights_detach.mean(0)[:, 1:, 1:]   # B * N-1 * N-1
        ####
        ...
        if method == 'bnr':
            ...
            cams_bn = torch.matmul(patch_attn.unsqueeze(1), cams.view(cams.shape[0],
                                                                             cams.shape[1], -1, 1)). \
                reshape(cams.shape[0], cams.shape[1], h, w)
            fine_cam_pred = self.avgpooling(cams_bn).flatten(1)
            return conv_logits, trans_logits, cams_bn, fine_cam_pred
