...
if __name__ == '__main__':
    ...
    parser.add_argument("--session_name", default="BNR", type=str)
    ...
    for ep in range(args.max_epoches):
        for iter, pack in enumerate(train_data_loader):
            ...
            logits_conv, logits_trans, cams, fine_cams = model('bnr', img)

            loss_cams = F.multilabel_soft_margin_loss((logits_conv + logits_trans).unsqueeze(2).unsqueeze(3)[:, 1:, :, :], label[:, 1:, :, :])
            loss_fine_cams = F.multilabel_soft_margin_loss(fine_cams.unsqueeze(2).unsqueeze(3)[:, 1:, :, :], label[:, 1:, :, :])
            
            loss  = loss_cams + loss_fine_cams
